...
class NewequipmentTest < ApplicationSystemTestCase
  fixtures :all
  ...
  test "test_3: logged in user functionality - details card and new equipment" do
    sign_in(@user)
    visit new_equipment_path
    assert_current_path new_equipment_path
    ...
    attach_file("equipment[files]", Rails.root + "test/fixtures/files/r_l_burnside.png")
    ...
    assert_current_path equipment_path(equipment.last)

    # destroy the equipment - click delete button
    click_button "Delete"
    ...
    assert_current_path equipments_path
  end
end
